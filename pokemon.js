...
//console.log(pokemon.pokemon[0].next_evolution[0].name)
let pokemoninfo = (name) => {
    let info;
    for (i in pokemon.pokemon) {
        if (name.toLowerCase() === pokemon.pokemon[i].name.toLowerCase()) {
            info = pokemon.pokemon[i]
            break;
        } else {
            console.log(`Pokemon ${name} doesn't exist`)
        }
    }
    return info;
}

// let a = console.log(pokemoninfo("Ivysaur"))
...
let pokemoneve = (eve) => {
    let e;
    for (i in pokemon.pokemon) {
        if (eve === pokemon.pokemon[i].name) {
            let a = (pokemon.pokemon[i].next_evolution);
            for (len = 0; len < a.length; len++) {
                e = `Pokemon ${eve} will evolve into ${a[len].name}`;
                console.log(e)
            }
            // return e;
        }
...
let weak = (we) => {
    ...
    for (let i = 0; i < (pokemon.pokemon).length; i++) {
        name = pokemon.pokemon[i].weaknesses;
        // console.log(name[1])
        r = pokemon.pokemon[i].name;
        for(let j =0; j < name.length;j++){
            if(we === name[j] ){
               
                 console.log(`${we} is weekness of ${r}`)
            }
        }
    }
